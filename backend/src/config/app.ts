...
// Version: 1.1
// Version: 1.1
// Fixed TypeScript error: Use bracket notation for process.env access per index signature requirements
 ...
 * Always returns a proper array, never undefined or non-iterable
 */
function parseArrayEnv(envVar: string | undefined, fallback: string[]): string[] {
  ...
  try {
    ...
    return result.length > 0 ? result : Array.from(fallback)
  } catch (error) {
